...
src/overlay/         React UI inside the panel iframe
  ...
  SubjectTabStrip.tsx Rotated strip + click-vs-dblclick; the "+" is the shared `AddSubjectTabButton`. Wheel-scroll = NATIVE CSS scroll-snap ONLY (no JS): `snap-y snap-proximity` on the scroll container + `snap-start` on each trigger. Proximity keeps trackpad momentum smooth — the client's stated priority is "smooth scrolling first, snapping second" — AND, unlike `snap-mandatory` (which snapped perfectly but glitched the mouse wheel "works then stops"), doesn't yank partial wheel scrolls back. Programmatic cue/select scrolls (`scrollToFirstLetter` AIR-2, `scrollToTab` NN-10) use `scrollTo({behavior:"smooth"})` to tab offsets, which are snap points. NOTE: the "works then stops" glitch was reported only on WINDOWS mouse wheels (Mac trackpad+wheel are smooth — chunky line-mode Windows deltas are what trip mandatory snap); if proximity isn't enough on Windows, gate a Windows-only wheel handler on `IS_WINDOWS` (src/lib/platform.ts) so the Mac pure-CSS path stays untouched. (History: a JS `scrollend` settle caused a ~1s wait + 2-step; one-tab-per-notch capped speed; a custom rAF glide fought trackpad momentum; a proximity+safety-net hybrid risked a delayed correction — all removed in favor of pure native snap.) AlphabetIndexRollout.tsx A–Z rail: clicking a letter CUES it — scrolls the first tab starting with that letter to the TOP and does NOT select it (client AIR rule); the previously-selected tab stays selected with its notes visible (scrolled out of view). Blue letter = SELECTED subject's letter (derived from activeSubjectTabId); matching letters hover-cue.
