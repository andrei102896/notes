...
src/overlay/         React UI inside the panel iframe
  ...
  BrandLockup.tsx    Shared NN logo/wordmark; `BrandLogo(viewBox?)` renders the two-N mark     NnModalFrame.tsx shared dialog shell (Cancel/OK buttons; width = Figma MODAL BG 577px; body `min-h` 214px, content vertically centered) + `ModalWatermark` — the dimmed faint NN behind the body, built from the wide `N_LEFT`/`N_RIGHT` Figma glyph assets laid side-by-side (Figma "UI LOGO" 401.78×97 box; ~71% of body width, centered, natural aspect — NOT the narrow header `BrandLogo`); `-z-10` inside `relative isolate overflow-hidden`; opacity 0.03 on modals, 0.15 on the note. Reused by every dialog + the note body; the empty-state panel (DashboardContent) uses the same watermark with a taller `min-h` 252px body (the two Figma modal heights).
