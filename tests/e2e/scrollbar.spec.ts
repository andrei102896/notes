...
test("notes list shows the accent scrollbar once it overflows", async ({
  ...
  expect(
    (geom.gutter / rootPx) * 16,
    "thumb + 8px gutter at the 16px reference",
  ).toBeCloseTo(14, 0);
  ...
  const edgeGapPx = await page.evaluate(
    async (dataUrl) => {
  ...
  expect(edgeGapPx, "accent thumb found in the gutter strip").toBeGreaterThan(
    0,
  );
  ...
  console.log(
    `thumb rows: head ${thumb.head}, mid ${thumb.mid}, tail ${thumb.tail}`,
  );
  expect(thumb.mid, "thumb has a body").toBeGreaterThan(2);
  expect(thumb.head[0], "top end is rounded, not square").toBeLessThan(
    thumb.mid,
  );
  expect(
    thumb.tail[thumb.tail.length - 1],
    "bottom end is rounded, not square",
  ).toBeLessThan(thumb.mid);
