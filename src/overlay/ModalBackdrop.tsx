...
export function ModalLogoBox({
  ...
  return (
    ...
    >
      ...
      <g transform={`translate(0 15.5) scale(1 ${MODAL_GLYPH_SQUEEZE_Y}) translate(0 -15.65)`}>
        <path
...
/** Flanking translucent bands on the metal bar (Figma UPDATE HEADER BOX LEFT/RIGHT). */
const HEADER_BAND_CLASS =
  "relative flex h-full flex-1 items-center border-[0.5px] border-accent-deep bg-accent/[0.86] shadow-[inset_0_-1.125rem_0.7rem_0.1875rem_rgba(0,0,0,0.28)]";
...
export function ModalBackdrop({
  ...
  return (
    <div className="absolute inset-0 overflow-hidden border-x-2 border-b-2 border-accent bg-[#333333]">
      {/* BG SQUARES: 3×5 grid, right column thin-bordered, middle row darker (Figma GRAY OUT BUY SCREEN series). */}
      <div
        aria-hidden
        className="absolute top-1/2 left-1/2 grid -translate-x-1/2 -translate-y-1/2 grid-cols-3 gap-x-[3.075rem] gap-y-[3.3125rem] opacity-65 blur-[1.6375rem]"
      >
      ...
      <div className="relative flex h-full flex-col">
        ...
        <div className="relative mx-[0.5625rem] mt-[0.5625rem] mb-[0.5625rem] flex-1 border border-accent-deep bg-accent/[0.07] backdrop-blur-[0.925rem]">
          ...
          <div className="absolute top-[32%] left-1/2 w-[74%] -translate-x-1/2 -translate-y-[75%]">
            ...
            <div className="absolute top-[67%] left-1/2 -translate-x-1/2 -translate-y-1/2 text-[4rem] leading-[5rem] whitespace-nowrap opacity-45">
              <span className="text-accent">NOTES</span>
              <span className="text-white">FOR</span>
              <span className="text-accent">NET</span>
            </div>
            {showTagline ? (
              <p className="absolute top-[113%] left-1/2 w-[110%] -translate-x-1/2 -translate-y-1/2 text-center text-[2.5rem] leading-[3.125rem] opacity-[0.64]">
                {TAGLINE_WORDS.map(({ word, white }, i) => (
                  <span
                    key={i}
                    className={white ? "text-white" : "text-accent"}
                  >
                    {i > 0 ? " " : ""}
                    {word}
                  </span>
